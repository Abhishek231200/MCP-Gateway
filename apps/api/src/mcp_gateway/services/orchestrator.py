...
class WorkflowOrchestrator:
    ...
    async def _planner_node(self, state: WorkflowState) -> dict[str, Any]:
        ...
        user_prompt = (
            ...
            f"   - When creating a Jira issue AND assigning it, pass 'assignee' (the accountId string) directly to 'create_issue' — do NOT add a separate 'assign_issue' step.\n"
            f"   - When the task says 'assign to accountId=XXX', extract only the accountId value (strip any surrounding text, parentheses, or newlines).\n"
